...
def main():
    ...
    try:
        ...
        phishing_prompt = (
            ...
            f"원본 메일 내용에 따라 맥락을 파악하여 템플릿 종류를 반환해줍니다. 템플릿 종류는 별도의 JSON 형식으로 함께 반환해 주세요.\n"
        ...
        '''
        # 템플릿 정보 추출
        template_info = phishing_response.get('choices', [{}])[0].get('message', {}).get('template', '').strip()
        '''
